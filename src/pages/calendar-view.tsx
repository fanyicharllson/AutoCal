    ...
    <div className="p-5">
      ...
      >
        {calendar.map((month) => (
          <div
            key={month.name}
            className="bg-white shadow-xl rounded-lg p-4 overflow-hidden"
          >
            <h2 className="text-xl font-bold text-center text-green-700 mb-3">
              {month.name}
            </h2>
            <div className="overflow-x-auto">
              <table className="w-full border-collapse border border-green-600 min-w-max">
                <thead>
                  <tr className="bg-green-700 text-white">
                    {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map(
                      (day) => (
                        <th key={day} className="p-2 border border-green-600">
                          {day}
                        </th>
                      )
                    )}
                ...
                <tbody>
                  {Array.from({ length: Math.ceil(month.days.length / 7) }).map(
                    (_, weekIndex) => (
                      <tr key={weekIndex}>
                        {month.days
                          ...
                          .map((day, index) => (
                            <td
                              key={index}
                              className={`p-2 border border-green-600 text-center ${
                                day.day === 0
                                  ? "bg-gray-200"
